...
import com.careercolony.neo4jServices.routes.ProfileService
...
class StartNeo4jServer(implicit val system: ActorSystem,
                       implicit val materializer: ActorMaterializer) extends ProfileService {
  

  def startServer(address: String, port: Int) = {
    Http().bindAndHandle(profileRoutes, address, port)
  }
}
